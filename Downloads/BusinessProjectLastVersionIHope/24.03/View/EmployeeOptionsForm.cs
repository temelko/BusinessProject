...
namespace View
{
    public partial class EmployeeOptionsForm : Form
    {
        ...
        {
            InitializeComponent();
            List<Department> allDepartments = depController.GetAllDepartments().ToList();
            SelectDepartment_CmbBox.DataSource = allDepartments.Select(x => x.Name).ToList();
        ...
        {
            this.Visible = false;
            ChooseDepOrEmpForm chooseForm = new ChooseDepOrEmpForm();
            chooseForm.Visible = true;
        }
    }
}
