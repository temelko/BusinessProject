...
namespace Controller
{
    public class EmployeeController
    {
        ...
        public void RemoveEmployeeById(int id)
        {
            using (businessContext = new BusinessProjectDbContext())
            {
                Employee employee = businessContext.Employees.FirstOrDefault(x => x.EmployeeId == id);

                businessContext.Employees.Remove(employee);
            ...
            if (year%4!=0)
            {
                                ...
                                || date == "31"))
                {
                    flag = false;
                                ...
                                || date == "31")) //nqma smisyl ot tozi 2ri if (spored men trqbva da mu se promeni mqstoto)
                {
                    flag = false;
                }
            }
